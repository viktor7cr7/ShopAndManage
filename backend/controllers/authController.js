...
export async function createUser(req, res, next) {
    ...
    const origin = process.env.ORIGIN_URL
        ? `${process.env.ORIGIN_URL}/verify-email/user`
        : 'http://localhost:5173/verify-email/user';
    try {
        const checkUser = await dbConnect.oneOrNone(
...
export const verifyEmailUser = async (req, res, next) => {
    ...
    await dbConnect.none(
        'UPDATE users SET verificationtoken = $1, verified = $2, isverified = $3',
        ['', new Date(), true]
    );
...
export async function loginUser(req, res, next) {
    ...
    if (!isValidUser) return next(new UnauthenticatedError('Неверные данные!'));

    if (!user?.isverified) {
        return next(
            new BadRequestError('Подтвердите адрес электронной почты!')
        );
    }
...
export const forgotPassword = async (req, res, next) => {
    ...
    if (!user) {
        return next(
            new BadRequestError('Аккаунта с данным email нет в системе')
        );
    }
    ...
    if (user) {
        ...
        const origin = process.env.ORIGIN_URL
            ? `${process.env.ORIGIN_URL}/reset-password/user`
            : 'http://localhost:5173/reset-password/user';
        ...
        await dbConnect.none(
            ...
            [hashString(passwordToken), passwordTokenExpirationDate, email]
        );
    }

    res.status(StatusCodes.OK).json({
        msg: 'Пожалуйста проверьте свою почту для сброса пароля',
    });
};
...
export const forgotPasswordAdmin = async (req, res, next) => {
    ...
    if (user) {
        const passwordToken = crypto.randomBytes(70).toString('hex');
        const origin = process.env.ORIGIN_URL
            ? `${process.env.ORIGIN_URL}/reset-password/admin`
            : 'http://localhost:5173/reset-password/admin';
        await sendResetPassswordEmail({
            name: user.name,
            email: user.email,
            token: passwordToken,
...
export const resetPasswordAdmin = async (req, res, next) => {
    ...
    try {
        ...
        if (user) {
            ...
            res.status(StatusCodes.BAD_REQUEST).json({
                msg: 'Данные устарели, запросите восстановление пароля ещё раз',
            });
        }
    } catch (error) {
        return next(new ErrorFromDataBase(error.message));
...
export async function createAdminUser(req, res, next) {
    ...
    const verificationToken = crypto.randomBytes(40).toString('hex');
    const origin = process.env.ORIGIN_URL
        ? `${process.env.ORIGIN_URL}/verify-email/admin`
        : 'http://localhost:5173/verify-email/admin';
    try {
        const checkUser = await dbConnectAdmin.oneOrNone(
            'SELECT * from users where email = $1',
            [email]
        );
        if (checkUser) {
...
export async function getCurrentAdminUser(req, res) {
    ...
    const user = await dbConnectAdmin.one(
        'SELECT author_id, name, email, avatar from users where author_id = $1',
        [authorId]
    );
