...
export async function getAllUserProducts(req, res, next) {
    ...
    if (sort) {
        ...
        orderBy =
            sortParams.length > 0 ? `ORDER BY ${sortParams.join(', ')}` : '';
    }
    ...
    const rangeFilter =
        minPrice && maxPrice
            ? `AND new_price BETWEEN ${minPrice} AND ${maxPrice}`
            : '';
    const filtersQuery =
...
export async function getOrders(req, res, next) {
    ...
    try {
        ...
        INNER JOIN
        orders o
        ON o.order_id = oi.order_id where o.user_id = $1`,
            [userId]
        );
...
export async function getItems(req, res, next) {
    ...
    INNER JOIN orders o
    ON oi.order_id = o.order_id where oi.order_id = $1`,
            [id]
        );
...
export async function getCurrentUser(req, res, next) {
    const { userId } = req.user;
    try {
        const user = await dbConnect.one(
            'SELECT u.name, u.id, u.image_url, u.email, b.amount from users u INNER JOIN balances b ON u.id = b.user_id where u.id = $1',
            [userId]
        );
...
export async function getTransaction(req, res, next) {
    const { userId } = req.user;
    try {
        const transactionItems = await dbConnect.any(
            'select tb.transaction_balances_id, tb.amount, tb.status, tb.created_at from transaction_balances tb INNER JOIN balances b ON tb.balance_id = b.balance_id where b.user_id = $1',
            [userId]
        );
...
export async function getSummAmount(req, res, next) {
    const { userId } = req.user;
    try {
        const totalAmount = await dbConnect.any(
            'select SUM(total_price) from orders where user_id = $1 and status = $2',
            [userId, 'paid']
        );
...
export async function getCurrentRating(req, res, next) {
    ...
    try {
        const rating = await dbConnect.oneOrNone(
            'select rating from review_product where order_item_id = $1 and user_id = $2',
            [itemId, userId]
        );
