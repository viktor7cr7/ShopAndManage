    ...
    try {
        ...
        if (!product) {
            res.status(StatusCodes.NOT_FOUND).json({
                msg: `Товара с id ${id} не существует`,
            });
        } else {
...
export async function setDiscountProduct(req, res, next) {
    ...
    if (!percentage || !start_date || !end_date) {
        return next(
            new BadRequestError('Укажите все обязательные атрибуты в запросе')
        );
    }

    try {
        const checkDiscount = await dbConnectAdmin.oneOrNone(
            'SELECT discount_id from discounts where product_id = $1',
            [id]
        );
        if (checkDiscount) {
        ...
        res.status(StatusCodes.CREATED).json({
            msg: 'Скидка успешно установлена',
        });
    } catch (error) {
        return next(new ErrorFromDataBase(error.message));
    }
    ...
    try {
        const check = await dbConnect.tx(async (t) => {
            ...
            if (checkOrder.status === 'unpaid')
                return next(
                    new BadRequestError(
                        'Неоплаченные товары невозможно оценивать'
                    )
                );
            if (checkOrder.user_id !== userId)
                return next(
                    new BadRequestError(
                        'Нельзя оставлять рейтинг для чужих ордеров'
                    )
                );

            const checkRatintItem = await t.any(
            ...
            if (checkRatintItem.length > 0) {
                return next(
                    new BadRequestError('Вы уже оценивали данный заказ')
                );
            }
            await t.none(
            ...
            const avgRating = await t.oneOrNone(
                `select ROUND(AVG(rating), 2) from review_product rp INNER JOIN order_items oi ON rp.order_item_id = oi.item_id WHERE oi.product_id = ${productId}`
            );

            await dbConnectAdmin.none(
                `UPDATE products SET rating = $1, review_count = review_count + 1 where product_id = $2`,
                [avgRating.round, productId]
            );
        });
