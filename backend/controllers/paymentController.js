    ...
    async function createOrder(items, userId, totalPrice) {
        try {
            if (paymentMethod === 'Balance') {
                const checkBalance = await dbConnect.one(
            ...
            const orderId = await dbConnect.tx(async (t) => {
                const order = await t.one(
                    ...
                    [
                        userId,
                        Number(totalPrice / DOLLAR_EXCHANGE_RATE).toFixed(2),
                        'unpaid',
                    ]
                );
                ...
                const queries = items.map((item) => {
                    const result =
                        Math.round(+item.price) / DOLLAR_EXCHANGE_RATE;
                    return t.none(
                        ...
                        [
                            ...
                            item.name,
                            item.quantity,
                            Number(item.price / DOLLAR_EXCHANGE_RATE).toFixed(
                                2
                            ),
                        ]
                    );
    ...
    switch (paymentMethod) {
        case 'Credit Card':
            const session = await stripe2.checkout.sessions.create({
                ...
                line_items: items.map((item) => ({
                    price_data: {
                        ...
                        unit_amount: Math.round(
                            (item.price * 100) / DOLLAR_EXCHANGE_RATE
                        ),
                    },
                ...
                success_url: process.env.ORIGIN_URL
                    ? `${process.env.ORIGIN_URL}/dashboard/user/orders`
                    : 'http://localhost:5173/dashboard/user/orders',
                cancel_url: process.env.ORIGIN_URL
                    ? `${process.env.ORIGIN_URL}/dashboard/user/all-products`
                    : 'http://localhost:5173/dashboard/user/all-products',
                metadata: {
                    orderId: orderId,
                    infoQuntityAndIdProduct: JSON.stringify(
        ...
        case 'Balance':
            ...
            await dbConnectAdmin.tx(async (t) => {
                ...
                const queries = infoQuntityAndIdProduct.map((item) => {
...
export async function addFundsUseStripe(req, res, next) {
    ...
    if (!amount)
        return next(new BadRequestError('Сумма не должна быть пустой'));
    ...
    try {
        const session = await stripe2.checkout.sessions.create({
            ...
            mode: 'payment',
            success_url: process.env.ORIGIN_URL
                ? `${process.env.ORIGIN_URL}/dashboard/user/transaction`
                : 'http://localhost:5173/dashboard/user/transaction',
            cancel_url: process.env.ORIGIN_URL
                ? `${process.env.ORIGIN_URL}/dashboard/user/add-funds`
                : 'http://localhost:5173/dashboard/user/add-funds',
            metadata: {
                userId,
                type: 'balance_top_up',
