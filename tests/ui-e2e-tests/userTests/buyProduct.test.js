...
test.describe('Покупка товара', () => {
  ...
  test('Покупка товара при использовании Stripe ', async () => {
    ...
    const totalPriceConvertToDollar = (Number(totalPrice) / 87.9).toFixed(2).split('.')[0];
    ...
    const totalPriceStripe = textCurrentAmountStripe
      .replace(/[^0-9,.]/g, '')
      .split('.')[0]
      .replace(',', '');
    ...
    if (await stripePostalCodeInput.isVisible()) {
      ...
      console.log('Постал код не требуется, пропускаем...');
    }
    ...
    expect(orderStatus).toBe('Status: unpaid'); // поменять на paid после подключения stripe в докер

    await close();
  });

  test('Покупка товара при использовании баланса юзера', async () => {
    await updateBalance();
  ...
  test('Покупка товара из корзины при использовании Stripe ', async () => {
    const { valueName, close } = await createProduct();
    const { page } = await authCookieAndNewContextUser();
    const buyProductsPage = new BuyProducts(page);
    ...
    const totalPriceConvertToDollar = (Number(totalPriceCart) / 87.9).toFixed(2).split('.')[0];
  ...
  test('Покупка товара из корзины при использовании баланса юзера', async () => {
    await updateBalance();
