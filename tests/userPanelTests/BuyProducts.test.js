...
test.describe.skip('Проверка страницы покупки товаров', () => {
    ...
    test('Фильтрация товаров по поиску (search)', async () => {
        ...
        const allProductsMatch = await page.$$eval('.product-containter', products => products.map(product => product.textContent.toLowerCase())
        .every(text => text.includes('смартфон')));
    ...
    test('Фильтрация товаров по категории (Category)', async () => {
        ...
        expect(allProductsMatch.length).toBe(9)
    ...
    test('Сортировка товаров по цене (Price по возрастанию)', async () => {
        ...
        const allProductsMatch = await page.$$eval('.product-price', element => element.map(product_price => {
    ...
    test('Сортировка товаров по рейтингу (Rating по возрастанию)', async () => {
        ...
        const allProductsMatch = await page.$$eval('.product-containter', containers => containers.map(container => {
    ...
    if (boundingRigthSlider && trackBox) {
        ...
        const newX = startX + boundingRigthSlider.width * 0.1; // 10% от ширины
    ...
    await page.waitForURL(/minPrice=0&maxPrice=8929/);

    const getValueRigthSlider = await page.$$eval('.track-values > div:nth-child(2)', element => element.map(el => +el.getAttribute('aria-valuenow')))
    const allProductsMatch = await page.$$('.product-containter')
    expect(...getValueRigthSlider).toBe(8929)
    expect(allProductsMatch.length).toBe(8)
    ...
    test('Сброс фильтров', async () => {
        ...
        await page.waitForURL('http://localhost:5173/dashboard/user/all-products')
