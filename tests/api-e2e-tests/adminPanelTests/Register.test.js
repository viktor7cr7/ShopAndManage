...
import { deleteAdminByEmail, deleteUserByEmail } from '../../dbUtils';
...
test.describe.skip('Регистрация пользователя + активация аккаунта через письмо', () => {
 ...
 ({isverified, verificationtoken} = await dbConnectAdmin.one('SELECT isverified, verificationtoken from users where email = $1', [dataRegiser.email]))
  expect(isverified).toBe(false)
  expect(verificationtoken).toBeTruthy()
  expect(typeof verificationtoken).toBe('string')

  response = await request.post(`/api/v1/auth/verify-email/admin?token=${verificationtoken}&email=${dataRegiser.email}`)
...
await deleteAdminByEmail(dataRegiser.email)
  ...
  test('Регистрация с некорректным email', async ({ request }) => {
    ...
    const response = await request.post('/api/v1/auth/admin/register', {data:dataRegiser
    })
