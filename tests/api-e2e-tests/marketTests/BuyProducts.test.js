...
test.describe('Покупка товаров', () => {
    ...
    test.skip('Покупка товара (через Stripe)', async ({request}) => {
        ...
        const postBuyProducts = await request.post('/api/v1/create-checkout-session', {
            data: {
                items: itemsProduct,
        ...
        const idProducts = JSON.parse(infoQuntityAndIdProduct).map(product => product.product_id)
        .join(',')
...
where p.product_id IN (${idProducts}) ORDER BY p.product_id`)
        ...
        const totalPriceConvertToDollar = (totalAmount / 87.90).toFixed(2)
        const dataWebhook = {
        ...
        const {total_price, status} = await dbConnect.one('SELECT total_price, status from orders where order_id = $1', [orderId])
        
        expect(+total_price).toBe(+totalPriceConvertToDollar)
        expect(status).toBe('paid')

        const checkChangeQuantity = await dbConnectAdmin.any(`SELECT product_id, stock_quantity from products where product_id IN (${idProducts}) ORDER BY product_id`)
    ...
    test.skip('Покупка товара (баланс юзера)', async ({request}) => {
        const cookies = await loginUser('victor@mail.ru', 'BETejEmm321')
        const cookieHeader = cookies.join('; ');
        const getProducts = await request.get('/api/v1/products', {
            headers: {
        ...
        const startBalance = await dbConnect.one('SELECT amount from balances where user_id = $1', user.id)
...
from products p 
LEFT JOIN discounts d ON p.product_id = d.product_id AND NOW() >= d.start_date AND NOW() <= d.end_date
where p.product_id IN (34,35,36) ORDER BY p.product_id`)
        ...
        const {total_price, status} = await dbConnect.one('SELECT total_price, status from orders where order_id = $1', '366')
        
        expect(+total_price).toBe(+totalPriceConvertToDollar)
        expect(status).toBe('paid')

        const checkChangeQuantity = await dbConnectAdmin.any(`SELECT product_id, stock_quantity from products where product_id IN (34,35,36) ORDER BY product_id`)
      ...
      const currentBalance = await dbConnect.one('SELECT amount from balances where user_id = $1', user.id)
