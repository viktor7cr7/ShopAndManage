...
test.describe('Проверка страницы покупки товаров', () => {
  ...
  test('Фильтрация товаров по поиску (Search)', async () => {
    ...
    const matches = await page.$$eval('.product-name', (products) =>
      products.map((product) => product.textContent.toLowerCase()).every((product) => product.includes('смартфон')),
    );
  ...
  test('Фильтрация товаров по категории (Category)', async () => {
    ...
    const matches = await page.$$eval('.product-category', (products) =>
      products.map((product) => product.textContent.toLowerCase()).every((product) => product.includes('электроника')),
    );
  ...
  test('Фильтрация товаров по статусу (в наличии) (Status = Avaliable)', async () => {
    ...
    const matches = await page.$$eval('.product-status', (products) =>
      products.map((product) => product.textContent.toLowerCase()).every((product) => product.includes('available')),
    );
  ...
  test('Фильтрация товаров по статусу (не в наличии) (Status = Ended)', async () => {
    ...
    const matches = await page.$$eval('.product-status', (products) =>
      products.map((product) => product.textContent.toLowerCase()).every((product) => product.includes('ended stock')),
    );
  ...
  test('Сортировка товаров по цене (Price по возрастанию)', async () => {
    ...
    for (let i = 0; i < (await itemsProduct.count()); i++) {
      const priceText = (await itemsProduct.nth(i).locator('.product-price').isVisible({ timeout: 500 }))
        ? await itemsProduct.nth(i).locator('.product-price').textContent()
        : await itemsProduct.nth(i).locator('.fa-percentage').textContent();
