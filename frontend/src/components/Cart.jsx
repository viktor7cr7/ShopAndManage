    ...
    <>
      ...
      <Modal isOpen={isOpen}>
        <ModalHeader>
          ...
          <CloseButton onClick={toggleCart} className="close-cart">
            X
          </CloseButton>
        </ModalHeader>
        <ModalContent className="content-cart">
          {cartItems.length === 0 ? (
            ...
            cartItems.map((item, key) => (
              <CartItem key={key} item={item} updateQuantity={updateQuantity} handleRemoveItem={removeItem} />
            ))
          )}
        </ModalContent>
        <ModalFooter>
          ...
          ></FormRowSelect>
          <button className="buy-btn" type="button" onClick={sendDataItems}>
            Buy Products
          </button>
        </ModalFooter>
      </Modal>
