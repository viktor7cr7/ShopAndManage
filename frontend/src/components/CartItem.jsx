      ...
      <ItemQuantity style={{ width: '20%' }}>
        ...
        {isEditing ? (
          <>
            ...
            />
            <button onClick={handleQuantitySubmit} className="update-btn">
              <FaCheck style={{ color: 'green', fontSize: '24px' }}></FaCheck>
            </button>
          </>
        ) : (
          <>
            {item.quantity}{' '}
            <button type="button" className="edit-quantity" onClick={handlePencilClick}>
              <FaPencilAlt style={{ height: 20, width: 20 }} />
            </button>
          </>
        )}
      </ItemQuantity>
      <button onClick={() => handleRemoveItem(item.key)} className="btn-action">
        <FaTrash style={{ height: 20, width: 20 }} />
      </button>
    </CartItemWrapper>
  );
};
