...
const Profile = () => {
  ...
  const infoGrade = {
    summAmount: Math.round((totalValue || 0) * 87.9),
  ...
  if (infoGrade.summAmount <= 25000 && infoGrade.summAmount !== 0) {
    ...
    (infoGrade.currentLevel = 'Профессионал'), (infoGrade.nextPriceLevel = 50000 - infoGrade.summAmount);
  } else if (infoGrade.summAmount >= 50000) {
    (infoGrade.currentLevel = 'Лидер'), (infoGrade.nextPriceLevel = 'Максимальный уровень');
  }
    ...
    <Wrapper>
      <Form method="post" className="form" encType="multipart/form-data">
        ...
        <div className="form-center">
          ...
          <button className="btn btn-block form-btn" id="saveChanges" disabled={isSubmitting}>
      ...
      <div className="grades-container">
        ...
        <div className="info-grades">
          <div className="info-text">
            Ваша сумма покупок: <span className="info-value">{infoGrade.summAmount} руб</span>
          </div>
          <div className="info-text">
            Ваш уровень: <span className="info-value">{infoGrade.currentLevel}</span>
          </div>
          <div className="info-text">
            До следующего уровня:{' '}
            <span className="info-value">
              {infoGrade.summAmount >= 50000 ? infoGrade.nextPriceLevel : infoGrade.nextPriceLevel + ' руб'}
            </span>
          </div>
          <div className="info-text">
            Персональная скидка на все товары: <span className="info-value-discount">{discount}%</span>
          </div>
        </div>
      </div>
    </Wrapper>
